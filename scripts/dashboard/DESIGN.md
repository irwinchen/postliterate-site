...
- Phase 3 — Vault Watch. **Done (2026-05-08; Outstanding Sources rule generalised 2026-05-09).** `scripts/dashboard/sources/vault-watch.mjs` covers four sub-streams:
  - **Outstanding sources** — items across the watched 01_Sources/ subfolders (`PDFs/`, `Podcasts/`, `Reports/`, `Clippings/`, `Transcripts/`, `Videos/`) plus loose files in the 01_Sources/ root, that no `type: source` Source Note references via frontmatter wikilink. Source Notes are identified by walking every `.md` under `01_Sources/**` and filtering on frontmatter `type: source`; their frontmatter is then scanned for any `[[Target]]` wikilink (under any key — generalises the original `pdf:` convention to also cover `audio:`, `video:`, `transcript:`, etc.) and both the literal target and its stem (extension-stripped) are added to a claimed-name set, lowercased. An item is outstanding if neither its full filename nor its stem appears in that set. Source Note `.md` files are themselves not items (they ARE the note). Clippings (`type: clipping`) are NOT Source Notes — each clipping needs a `type: source` note written for it, so they show as outstanding until that happens. Output shape: `{ count, items_total, source_notes_total, items: [{ path, name, kind, size_kb, mtime }] }` where `kind` is the watched-folder name (or `Loose`).
  ...
  UI: KPI row (outstanding/queue/latest daily), then Outstanding + Queue full-width lists, then a 2-col Daily/Inbox grid. Outstanding rows show a kind badge (`PDFs`, `Podcasts`, `Clippings`, etc.) via `.vw-tag`. Every row links via `obsidian://open?vault=PostLiterate&file=<path>` so clicks open the right note in Obsidian on whichever machine the dashboard is being viewed from. New `.vw-*` CSS block in `admin-ui.html`. KPI overview unchanged — still counts non-null sections.
...
- Phase 7 — MacBook hourly Cowork digest job (writes to `vault/06_Meta/Sessions/`). **Pending / optional.** Since editing happens on the MacBook (appliance model, see `CLAUDE.md`), capturing MacBook Cowork sessions into the vault is the way the Mini's dashboard sees that work. The vault-sessions stream remains in the snapshot but stays empty unless a digest writer is added.
...
- **Auto git-pull on Mini is ON** (appliance model). The `org.postliterate.git-pull` timer fast-forwards from `origin/main` every 30 min. It was briefly disabled 2026-05-09 when the Mini was used as a dev host, then re-enabled 2026-05-19 when that was reverted. `git-pull.sh` now also self-heals a dirty working tree by stashing before pulling (see the script header), so the dashboard rewriting cache files can no longer stall the sync.
...
- **Mini is an appliance** (reverted from dev-host on 2026-05-19). Auto git-pull is ON (every 30 min); all editing happens on the MacBook. See `CLAUDE.md` → Two-Machine Workflow.
...
- **Figure PNG churn (TODO).** `scripts/figure-screenshots.mjs` re-renders the figure PNGs in `public/images/figures/`. Even when source content is unchanged, a fresh render can differ at the byte level (subpixel/AA non-determinism), so every run produces tracked diffs on files that must stay in git (the live site serves them as static assets). Fix: before writing each PNG, compare the new bytes against the existing file (or compare a perceptual/structural hash) and skip the write if effectively identical — so only genuine visual changes hit the index. The `.figure-hashes.json` manifest already gates *whether* to re-screenshot a source; this adds a second gate on *whether the output actually changed*. (Untracking the PNGs is not an option — the deployed site needs them committed.)
